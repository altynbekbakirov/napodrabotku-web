...
class CountryController extends Controller
{
    ...
    public function create()
    {
        $title = 'Страны ';
        $country = new Country();
        return view('admin.countries.create', compact('title', 'country'));
    }

    public function store(Request $request)
    {
        $this->validate($request, [
            'nameKg' => ['required', 'unique:countries'],
            'nameRu' => ['required', 'unique:countries'],
        ]);
    ...
    {
        $title = 'Страны - '.$country->nameRu;
        return view('admin.countries.show', compact('title'));
    }
    ...
    public function update(Request $request, Country $country)
    {
        ...
        return redirect()->route('countries.index', $country);
    }
    ...
    public function api(Request $request)
    {
        ...
        $resultPaginated = Country::orderBy('id', 'asc')->orderBy('id', 'asc');
        ...
        foreach ($resultPaginated as $key => $row) {
            ...
            $row->actions = '
                <a href="'.route('countries.edit', $row).'" class="btn btn-sm btn-clean btn-icon mr-2" title="Редактировать">
                ...
                <a href="'.route('countries.delete', $row).'" class="btn btn-sm btn-clean btn-icon" title="Удалить">
                    <span class="svg-icon svg-icon-md">
                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
                            <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
